...
{
    public class NonDurableStockInController : BaseController
    {
        ...
        {
            CreateDropDown();
            return View("Form", new NondurableStockInVM());
        }
        ...
        private NondurableStockInVM CreateModel(NondurableStockIn model)
        {
            var _items = db.DurableStockInItems.Where(i => i.IsActive == true && i.StockInID == model.StockInID).ToList();
            return new NondurableStockInVM()
            {
                ...
                NondurableStockInItems = db.NondurableStockInItems.Where(i => i.IsActive == true && i.StockInID == model.StockInID)
                .ToList().Select(i => new NondurableStockInItemVM
                {
                    ID = i.ID,
        ...
        [HttpPost]
        [AccessControl("Add")]
        public JsonResult Insert(Non_Durable_StockIn_Form_VM model)
        {
            ...
            using (var trans = db.Database.BeginTransaction())
            {
                try
                {
                    var _stockIn = new NondurableStockIn
                    {
                        M7Number = model.M7Number,
                        OrderNumber = model.OrderNumber,
                    ...
                    db.NondurableStockIns.Add(_stockIn);
                    db.SaveChanges();

                    db.ActivityLogs.Add(new ActivityLog
                    {
                        ModifiedTable = "NondurableStockIns",
                    ...
                    if (model.NondurableStockInItems != null)
                    {
                        foreach (var row in model.NondurableStockInItems)
                        {
                            if (row != null)
                            {
                                var _item = new NondurableStockInItem()
                                {
                                ...
                                db.NondurableStockInItems.Add(_item);
                                db.SaveChanges();
                                db.ActivityLogs.Add(new ActivityLog
                                {
                                    ModifiedTable = "NondurableStockInItems",
                                    ModfiedId = row.ID,
                                    Action = "Insert",
                                    UserId = AppUser.Id,
                                    ModifiedTime = DateTime.Now,
        ...
        public JsonResult ListPartial(Durable_StockIn_Search search)
        {
            ...
            var _Receipts = db.NondurableStockIns.Where(t => t.IsActive == true).OrderByDescending(s => s.StockInDate).ToList();
        ...
        {
            var _record = db.NondurableStockIns.Where(t => t.IsActive == true && t.StockInID == id).FirstOrDefault();
            if (_record == null)
            {
                return HttpNotFound();
            }
        ...
        {
            CreateDropDown();
            var _stockIn = db.NondurableStockIns.SingleOrDefault(t => t.StockInID == id);
            if (_stockIn == null)
            {
                return HttpNotFound();
            }
        ...
        public JsonResult SaveScanImage(FileUpload model)
        {
            ...
            try
            {
                ...
                string StockInScanFolder = ConfigurationManager.AppSettings["NondurableStockIn"].ToString();
